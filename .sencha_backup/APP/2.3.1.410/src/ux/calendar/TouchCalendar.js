 ...
 */
/**
 * @class Ext.ux.calendar.TouchCalendar
 * @author Stuart Ashworth
 *
 * For use with Sencha Touch 2
 * 
 * This extension wraps the Ext.ux.calendar.TouchCalendarView in a Ext.Carousel component and allows the calendar to respond to swipe
 * gestures to switch the displayed period. It works by creating 3 Ext.ux.calendar.TouchCalendarViews and dynamically creating/removing
 * views as the user moves back/forward through time. 
 * 
 * ![Ext.ux.calendar.TouchCalendar Screenshot](http://www.swarmonline.com/Ext.ux.TouchCalendar/screenshots/Ext.ux.TouchCalendar-month-ss.png)
 * 
 * [Ext.ux.calendar.TouchCalendar Demo](http://www.swarmonline.com/Ext.ux.TouchCalendar/examples/Ext.ux.TouchCalendar.html)
 * 
 */
Ext.define('Ext.ux.calendar.TouchCalendar',{
	...
	*/
	getViewConfig: function(viewValue){
		...
		if(this.getEnableSimpleEvents()){
			...
			plugins.push(Ext.create('Ext.ux.calendar.TouchCalendarSimpleEvents', config));
		} else if (this.getEnableEventBars()){
			var config = Ext.isObject(this.getEnableEventBars()) ? this.getEnableEventBars() : {};
			plugins.push(Ext.create('Ext.ux.calendar.TouchCalendarEvents', config));
		}
	...
	initViews: function(){
		...
		items.push(
		    Ext.create('Ext.ux.calendar.TouchCalendarView', Ext.applyIf({
		        currentDate: viewValue
		...
		// active view
		items.push(
			Ext.create('Ext.ux.calendar.TouchCalendarView', Ext.ux.calendar.TouchCalendarView(this.getViewConfig(origCurrentDate)))
		);

		// second out of view (i.e. third)
		viewValue = this.getViewDate(origCurrentDate, 1);
		items.push(
			Ext.create('Ext.ux.calendar.TouchCalendarView', Ext.ux.calendar.TouchCalendarView(Ext.applyIf({
		        currentDate: viewValue
		    }, this.getViewConfig(viewValue))))
		);
	...
	onActiveItemChange: function(container, newCard, oldCard){
		if (this.getEnableSwipeNavigate()) {
			...
			if (direction === 'forward') {
				...
				var newCalendar = new Ext.ux.calendar.TouchCalendarView(this.getViewConfig(this.getViewDate(newCard.currentDate, 1)));
